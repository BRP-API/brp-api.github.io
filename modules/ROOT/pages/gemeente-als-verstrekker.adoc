...
=== 3. Proxy
Voor autorisatie op detailniveau. Met welke rol of taak mag een medewerker of applicatie welke set gegevens opvragen? Hiervoor maak je gebruik van de link:/how-tos/personen-response-filteren.adoc[filtermogelijkheden] die de {apiname} biedt. Dit kun je regelen in jouw API Gateway, zoals in link:https://github.com/Amsterdam/haal-centraal-proxy[dit voorbeeld van de gemeente Amsterdam].
...
* het taaksysteem/de ambtenaar met een bepaalde taakautorisatie die de verstrekking heeft ontvangen;
...
* de BSN's van de personen die in het antwoord voorkomen. _Let op! het kan voorkomen dat jouw binnengemeentelijke afnemer het BSN niet vraagt. Zorg dus dat het BSN in het request van jouw API Gateway aan RvIG link:/how-tos/personen-response-filteren[altijd gevraagd] wordt!_ Dit kun je regelen in jouw API Gateway als intermediate endpoint.
